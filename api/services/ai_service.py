...
async def get_ai_response(question: str, context: Optional[List[str]] = None) -> str:
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": "You are Genie, a helpful AI assistant in a chat application. Keep responses concise and friendly."}
        ]
        
        # Add context if provided
        if context:
            context_text = "\n".join(context[-5:])  # Last 5 messages for context
            messages.append({"role": "system", "content": f"Recent conversation context: {context_text}"})
